...
import { addToFavorites, HandleCardClose } from "./LibraryProvider"
     ...
     { 
        if(favorite.varietalRegionId === wine.id){
            return <button key={favorite.id} className="">
               <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="" stroke="currentColor" className="fill-yellow-500 w-8 h-8">
...
const HandleCardClose = (event) => {
    ...
    <div className="absolute top-2 right-2">
            ...
            : ""
             ...
             {!rabbitUserObject.staff && favorites.every(favorite => favorite.varietalRegionId !== wine.id)? <button className="btn btn-sm bg-secondary shadow-2xl shadow-white text-white rounded-full font-extrabold text-xl" onClick={()=> addToFavorites(wineObject).then(()=>navigate("/cellar")) }> +
            </button> 
            :""
