...
const wineObject = {
    ...
    <div className="card w-64 h-64 bg-slate-100 shadow-xl m-2">
        ...
        <div className="card-body p-2 m-2 h-full">
            ...
            <Link className="p-1" to={`/library/details/${wine.id}`}>
            <img className="" src={wine?.varietal?.image}/>
            </Link>
            
        <div className="card-actions justify-end">
            {rabbitUserObject.staff ?  <button className="btn" onClick={()=> navigate(`/library/edit/${wine.id}`) }>Edit Details</button>
            : <button className="fill-yellow-200" onClick={()=> addToFavorites(wineObject) }><svg xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 ml-auto mr-auto m-4">
